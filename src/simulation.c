...
/*   Updated: 2022/04/06 21:22:24 by mmota            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*action(void *arg)
{
	...
	while (1)
	{
		if (philo->meals_count == 0 || !get_forks(sim, philo))
			break ;
		if (sim->dead || !eating(sim, philo) || is_dying(sim, philo))
			break ;
		if (sim->dead || !sleeping(sim, philo) || is_dying(sim, philo))
			break ;
		if (sim->dead || !thinking(sim, philo) || is_dying(sim, philo))
			break ;
	}
	return (0);
}
