...
/*   Updated: 2022/03/22 16:53:01 by mmota            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	sim->specs.n_philos = ft_atoi(argv[1]);
...
void	init_philos(t_sim *sim, long int start)
{
	...
	sim->philos = malloc(sizeof(t_philos) * sim->specs.n_philos);
	if (!sim->philos)
...
void	init_forks(t_sim *sim)
{
	...
	sim->philos[0].right_fork = &sim->philos[sim->specs.n_philos - 1].left_fork;
}
...
int	init_threads(t_sim *sim)
{
	...
	sim->threads = (pthread_t *)malloc(sizeof(pthread_t) * sim->specs.n_philos);
	if (!sim->threads)
